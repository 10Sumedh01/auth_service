...
def dashboard(request):
    apps = App.objects.filter(developer=request.user).annotate(
...
def create_app_dashboard(request):
    if request.method == "POST":
        ...
        try:
            app = App.objects.create(name=name, developer=request.user)
            api_key = ApiKey.objects.create(app=app)
            messages.success(request, f"App {name} created successfully!")
            return render(
                request,
                "auth_service/create_app.html",
                {"app_id": app.app_id, "api_key": api_key.key},
            )
        except Exception as e:
...
def user_list_dashboard(request, app_id):
    try:
        app = App.objects.select_related("developer").get(
            app_id=app_id, developer=request.user
        )
        users = (
            User.objects.filter(app=app)
            .values(
...
def add_auth_config(request, app_id):
    ...
    if request.method == "POST":
        ...
        if created:
            messages.success(
                request, f"{provider.capitalize()} OAuth configuration saved."
            )
        else:
            ...
            config.save()
            messages.success(
                request, f"{provider.capitalize()} OAuth configuration updated."
            )
...
def test_oauth_config(request, app_id, config_id):
    ...
    if request_count >= 5:
        messages.error(
            request,
            "You have reached the maximum number of test attempts. Please try again later.",
        )
        return redirect("app_details", app_id=app_id)
    ...
    config = get_object_or_404(
        OAuthConfig, id=config_id, app__app_id=app_id, app__developer=request.user
    )
    ...
    try:
        if config.provider == "google":
            ...
            if "access_token" in response.json():
                ...
                logger.error(
                    f"GitHub OAuth test failed for app {app_id}: {response.json()}"
                )
                messages.error(
                    request,
                    f"GitHub OAuth test failed: {response.json().get('error_description', 'Unknown error')}",
                )
        ...
        messages.error(
            request,
            "An error occurred during the OAuth test. Please check the logs for details.",
        )
