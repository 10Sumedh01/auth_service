...
urlpatterns = [
    # Authentication APIs
    path(
        "auth/credentials/signup/<str:app_id>",
        CredentialsSignUpView.as_view(),
        name="credentials_signup",
    ),
    path(
        "auth/credentials/signin/<str:app_id>",
        CredentialsSignInView.as_view(),
        name="credentials_signin",
    ),
    path("auth/magic-link/<str:app_id>", MagicLinkView.as_view(), name="magic_link"),
    path(
        "auth/verify/<str:app_id>",
        MagicLinkVerifyView.as_view(),
        name="magic_link_verify",
    ),
    path(
        "auth/<str:provider>/<str:app_id>",
        OAuthRedirectView.as_view(),
        name="oauth_redirect",
    ),
    path(
        "auth/callback/<str:provider>/<str:app_id>",
